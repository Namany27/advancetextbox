---
tags: [gradio-custom-component, textbox, advancetextbox, dewnload, download, downloadbtn, downloadaspdf, pdf, downloadintextbox]
title: gradio_advancetextbox
short_description: This is a textbox with some extra button not just copy
colorFrom: blue
colorTo: yellow
sdk: gradio
pinned: false
...
| name | description |
|:-----|:------------|
| `change` | Triggered when the value of the AdvanceTextbox changes either because of user input (e.g. a user types in a textbox) OR because of a function update (e.g. an image receives a value from the output of an event trigger). See `.input()` for a listener that is only triggered by user input. |
| `input` | This listener is triggered when the user changes the value of the AdvanceTextbox. |
| `select` | Event listener for when the user selects or deselects the AdvanceTextbox. Uses event data gradio.SelectData to carry `value` referring to the label of the AdvanceTextbox, and `selected` to refer to state of the AdvanceTextbox. See EventData documentation on how to use this event data |
| `submit` | This listener is triggered when the user presses the Enter key while the AdvanceTextbox is focused. |
| `focus` | This listener is triggered when the AdvanceTextbox is focused. |
| `blur` | This listener is triggered when the AdvanceTextbox is unfocused/blurred. |
| `stop` | This listener is triggered when the user reaches the end of the media playing in the AdvanceTextbox. |
| `copy` | This listener is triggered when the user copies content from the AdvanceTextbox. Uses event data gradio.CopyData to carry information about the copied content. See EventData documentation on how to use this event data |
| `download` | This listener is triggered when the user downloads a file from the AdvanceTextbox. Uses event data gradio.DownloadData to carry information about the downloaded file as a FileData object. See EventData documentation on how to use this event data |
